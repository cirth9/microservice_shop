...
// ParseToken 解析token
func ParseToken(tokenString string) (*GoodsBasicClaims, error) {
	zap.S().Info("tokenString  ", tokenString)
	token, err := jwt.ParseWithClaims(tokenString, &GoodsBasicClaims{}, func(token *jwt.Token) (i interface{}, e error) {
	...
	if token != nil {
		...
		claims, ok := token.Claims.(*GoodsBasicClaims)
		if ok && token.Valid {
