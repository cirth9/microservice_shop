...
func InitGoodsSrvMysqlConfig() {
	var err error
	dsn := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=utf8mb4&parseTime=True&loc=Local",
		config.TheServerConfig.MysqlConfig.GoodsName,
...
func InitConfig() {
	...
	goodsSrvConfig, err := client.GetConfig(vo.ConfigParam{
	...
	fmt.Println(goodsSrvConfig)
	err = json.Unmarshal([]byte(goodsSrvConfig), &config.TheServerConfig)
	if err != nil {
		zap.S().Panic(err)
	}

	zap.S().Info("TheServerConfig: ", config.TheServerConfig)
	InitGoodsSrvMysqlConfig()
}
