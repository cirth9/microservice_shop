...
func (s *GoodsServer) BrandList(ctx context.Context, req *proto.BrandFilterRequest) (*proto.BrandListResponse, error) {
	...
	result := global.MysqlDB.Scopes(Paginate(int(req.Pages), int(req.PagePerNums))).Find(&brands)
	if result.Error != nil {
		return nil, result.Error
	}

	var total int64
	global.MysqlDB.Model(&model.Brands{}).Count(&total)
...
func (s *GoodsServer) CreateBrand(ctx context.Context, req *proto.BrandRequest) (*proto.BrandInfoResponse, error) {
	//新建品牌
	if result := global.MysqlDB.Where("name=?", req.Name).First(&model.Brands{}); result.RowsAffected == 1 {
...
func (s *GoodsServer) DeleteBrand(ctx context.Context, req *proto.BrandRequest) (*emptypb.Empty, error) {
	if result := global.MysqlDB.Delete(&model.Brands{}, req.Id); result.RowsAffected == 0 {
...
func (s *GoodsServer) UpdateBrand(ctx context.Context, req *proto.BrandRequest) (*emptypb.Empty, error) {
	brands := model.Brands{}
	if result := global.MysqlDB.First(&brands); result.RowsAffected == 0 {
