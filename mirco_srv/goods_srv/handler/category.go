...
func (s *GoodsServer) GetAllCategorysList(context.Context, *emptypb.Empty) (*proto.CategoryListResponse, error) {
	...
	*/
	...
	global.MysqlDB.Where(&model.Category{Level: 1}).Preload("SubCategory.SubCategory").Find(&categorys)
...
func (s *GoodsServer) GetSubCategory(ctx context.Context, req *proto.CategoryListRequest) (*proto.SubCategoryListResponse, error) {
	...
	if result := global.MysqlDB.First(&category, req.Id); result.RowsAffected == 0 {
	...
	//}
	global.MysqlDB.Where(&model.Category{ParentCategoryID: req.Id}).Find(&subCategorys)
...
func (s *GoodsServer) CreateCategory(ctx context.Context, req *proto.CategoryInfoRequest) (*proto.CategoryInfoResponse, error) {
	...
	tx := global.MysqlDB.Model(&model.Category{}).Create(cMap)
	fmt.Println(tx)
...
func (s *GoodsServer) DeleteCategory(ctx context.Context, req *proto.DeleteCategoryRequest) (*emptypb.Empty, error) {
	if result := global.MysqlDB.Delete(&model.Category{}, req.Id); result.RowsAffected == 0 {
		return nil, status.Errorf(codes.NotFound, "商品分类不存在")
	}
